...
// 创建模拟文件列表
function createMockFileList(count: number = 5, size: number = 1024) {
  const fileList: Array<{ name: string, content: Buffer }> = [];
  
  for (let i = 1; i <= count; i++) {
    // 创建一些示例内容
    const content = Buffer.alloc(size, `This is file ${i} content.`);
...
// 创建示例ZIP文件
async function createZipArchive(fileCount: number, fileSize: number): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      ...
      archive.on('error', (err: any) => {
        console.error('创建ZIP归档时出错:', err);
        reject(err);
      });
      
      // 设置完成事件
      archive.on('end', () => {
        console.log(`ZIP归档创建完成，大小: ${archive.pointer()} 字节`);
      });
      ...
      // 创建模拟文件列表
      const fileList = createMockFileList(fileCount, fileSize);
      
      // 添加文件到ZIP
      fileList.forEach(file => {
        archive.append(file.content, { name: file.name });
        console.log(`添加文件到ZIP: ${file.name}, 大小: ${file.content.length} 字节`);
      });
      
      // 添加一个子文件夹和文件
      archive.append(Buffer.from('This is a file in subfolder'), { name: 'subfolder/test.txt' });
      ...
      console.error('创建ZIP归档时出错:', error);
      reject(error);
    }
  });
}
