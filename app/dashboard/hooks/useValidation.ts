...
export const useValidation = () => {
  ...
  // 验证整个表单
  const validateForm = (userInfo: UserInfo): boolean => {
    const newErrors: ValidationErrors = {};
    
    // 验证用户名
    const displayNameError = validators.required('用户名')(userInfo.displayName);
  ...
  // 验证单个字段
  const validateField = (field: keyof UserInfo, value: string): string | undefined => {
